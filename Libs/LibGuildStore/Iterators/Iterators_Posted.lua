...
function internal:iterateOverPostedItemsData(itemId, versionId, saleId, prefunc, loopfunc, postfunc, extraData)
  ...
  local versionList
  if not itemId then
    itemId, versionList = next(posted_items_data, nil)
    versionId = nil
  else
  ...
  while itemId do
    ...
    while versionId do
        ...
        while saleId do
          ...
          if saleId and (GetGameTimeMilliseconds() - checkTime > extraData.checkMilliseconds) then
            LibGuildStore_Internal:dm("Debug", string.format(
              "[iterateOverPostedItemsData] Breaking sales loop: time exceeded for saleId %d", saleId
            ))
            LibExecutionQueue:continueWith(function()
              internal:iterateOverPostedItemsData(itemId, versionId, saleId, nil, loopfunc, postfunc, extraData)
            end, "iterateOverPostedItemsData")
            return
          end
        end
      end
      ...
      local debugString = string.format("[iterateOverPostedItemsData] Breaking versionId loop: time exceeded for itemId %s, versionId %s", itemId or "nil itemId", versionId or "nil versionId")
    ...
    local itemData = posted_items_data[itemId]
...
function internal:InitPostedItemsHistory()
  ...
  local postfunc = function(extraData)
    ...
    if LibGuildStore_SavedVariables["showGuildInitSummary"] then
      internal:dm("Info", string.format(GetString(GS_INIT_POSTED_HISTORY_SUMMARY), GetTimeStamp() - extraData.start, internal.totalPosted))
    end
  end
...
function internal:ReferencePostedItemsDataContainer()
  ...
  for itemId, versionList in pairs(savedVars) do
    if not posted_items_data[itemId] then
        ...
        if hasSales then
          for saleId, saleData in pairs(versionData["sales"]) do
            if type(saleId) == "number" and type(saleData) == "table" and type(saleData.timestamp) == "number" then
              posted_items_data[itemId][versionId] = posted_items_data[itemId][versionId] or {}
