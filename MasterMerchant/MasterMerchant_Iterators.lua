    ...
    while (versionid ~= nil) do
      if versiondata['sales'] then
        ...
        while (saleid ~= nil) do
          ...
          --[[
          currentTask:Call(function() self:iterateOverSalesData(itemid, versionid, saleid, nil, loopfunc, postfunc, extraData, currentTask) end)
          ]]--
        end
...
function MasterMerchant:InitItemHistory(currentTask)
  ...
  if (extradata.doGuildItems or extradata.doMyItems or extradata.doGuildSales or extradata.doGuildPurchases) then
    ...
    local prefunc     = function(extraData, currentTask)
      ...
      self:setScanning(true)
      MasterMerchant:dm("Debug", "InitItemHistory prefunc")
    end
    ...
    local postfunc    = function(extraData, currentTask)
      ...
      self:setScanning(false)
      MasterMerchant:dm("Debug", "InitItemHistory postfunc")
...
function MasterMerchant:indexHistoryTables(currentTask)
  ...
  local prefunc    = function(extraData, currentTask)
    ...
    self.SRIndex                = {}
    self:setScanning(true)
    MasterMerchant:dm("Debug", "indexHistoryTables prefunc")
  end
  ...
  local postfunc   = function(extraData, currentTask)
    currentTask:Call(function() self:setScanning(false) end)
               :Then(function() MasterMerchant:dm("Debug", "indexHistoryTables postfunc") end)
...
function MasterMerchant:CleanOutBad()
  ...
  local prefunc  = function(extraData, currentTask)
    ...
    currentTask:Call(function() self:setScanning(true) end)
  end
