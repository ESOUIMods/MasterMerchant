...
function internal:iterateOverShoppinglistData(itemid, versionid, saleid, prefunc, loopfunc, postfunc, extraData)
  extraData.versionCount      = (extraData.versionCount or 0)
  extraData.idCount           = (extraData.idCount or 0)
  extraData.checkMilliseconds = (extraData.checkMilliseconds or 20)
    ...
    while (versionid ~= nil) do
      if versiondata['sales'] then
        ...
        while (saleid ~= nil) do
          ...
          if saleid and (GetGameTimeMilliseconds() - checkTime) > extraData.checkMilliseconds then
            local LEQ = LibExecutionQueue:new()
            LEQ:ContinueWith(function() internal:iterateOverShoppinglistData(itemid, versionid, saleid, nil, loopfunc,
              postfunc,
              extraData) end, nil)
            return
          end
      ...
      if versionid and (GetGameTimeMilliseconds() - checkTime) > extraData.checkMilliseconds then
        local LEQ = LibExecutionQueue:new()
        LEQ:ContinueWith(function() internal:iterateOverShoppinglistData(itemid, versionid, saleid, nil, loopfunc, postfunc,
          extraData) end, nil)
        return
      end
    end
...
function internal:TruncateShoppinglistHistory()
  internal:dm("Debug", "TruncateShoppinglistHistory")
  ...
  if not internal.isDatabaseBusy then
    internal:iterateOverShoppinglistData(nil, nil, nil, prefunc, loopfunc, postfunc, {})
  end
